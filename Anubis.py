...
class text_widget(QWidget):
    ...
    def itUI(self):
        ...
        ext=QTextEdit()
        text = QTextEdit()
        csharp_text = QTextEdit()
        Python_Coloring.P_CS_Highlighter(text)
        hbox = QHBoxLayout()
        hbox.addWidget(text)
        self.setLayout(hbox)
...
class Widget(QWidget):
    ...
    def on_clicked(self, index):
        ...
        if nn[0]:
            ...
            with f:
                data = f.read()
                if(ext.toPlainText()=='py' or ext.toPlainText()=='cs'):
                    Python_Coloring.setExt(ext.toPlainText())
                    text.setText(data)
